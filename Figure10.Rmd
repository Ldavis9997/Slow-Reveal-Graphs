...
area<- (ggplot(data= vote_data, aes(x= pop_groups, y= amount, group = vote_type, linetype=vote_type, color=vote_type))+ geom_line(size =1)+ geom_point(aes(color= vote_type), size=2.3)+ scale_color_manual(values=pop_colors, guide_legend(label.vjust=5))+ scale_linetype_manual(breaks = types, values=c(5,1,5,1,5), labels= types)+
...
new<- (area + theme_classic()+
  theme(
           ...
           legend.key.height= unit(.2, 'cm'), legend.key.width= unit(2.3, 'cm'),
...
+ scale_y_continuous(breaks = c(0,5,10,15,20,25), limits= c(0,25), expand= expansion(mult=c(0,.65))))
...
new1+ guides(color= guide_legend(override.aes=list(linetype=c(5,1,5,1,5))))
```