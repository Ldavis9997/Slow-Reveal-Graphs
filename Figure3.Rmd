...
area<- ((ggplot(vote_data, aes(x = pop_group, 
  ...
  geom_col(color = "black", size= .2, width=.7)+ geom_text(size = 4, 
            position = position_stack(vjust = 0.5), color=ifelse(vote_data$vote_type=="Hispanic", 'black', 'white')) + scale_fill_manual(values = pop_colors, guide= guide_legend(reverse=TRUE)))+
