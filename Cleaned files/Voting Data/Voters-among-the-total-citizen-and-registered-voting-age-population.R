...
# DESCRIPTION:The following data visualizations are from the U.S. Census Bureau and 
...
area <- ggplot(vote_data, aes(x = pop_group, y = amount)) +
  geom_col(aes(fill = vote_type), color = "black", size = .2, width=.9) +
...
ggsave('new1.png') #Margins when using R markdown should be: {r, fig.height= 5, fig.width=5}
