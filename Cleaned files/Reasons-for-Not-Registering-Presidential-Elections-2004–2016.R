...
area<- (ggplot(data= vote_data, aes(x= pop_groups, y= amount, group = vote_type, linetype=vote_type, color=vote_type))+ geom_line(size =1.1)+ geom_point(aes(color= vote_type), size=2.3)+ scale_color_manual(values=pop_colors, guide_legend(label.vjust=5))+ scale_linetype_manual(breaks = types, values=c(5,1,5,1,5), labels= types)+
...
ggsave("figure11.png") #markdown margins: {r,fig.height= 6, fig.width= 8}