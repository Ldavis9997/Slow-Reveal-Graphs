...
area<- (ggplot(data= vote_data, aes(x= pop_groups, y= amount, group = vote_type, color= vote_type))+ geom_line(aes(color=vote_type), size =1.1)+ geom_point(aes(color=vote_type),size=2.3)+ scale_color_manual(values=pop_colors, guide_legend(label.vjust=2))+
...
ggsave('figure9.png') #Markdown margins: {r,fig.height= 5, fig.width= 8}
