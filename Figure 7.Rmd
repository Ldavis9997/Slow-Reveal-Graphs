...
types= c("Less than high school", "High school diploma", "Some college or", "Bachelor's degree")
...
area<- (ggplot(vote_data, aes(x = pop_groups, 
                      ...
                      fill = fill, 
                      label = amount1)) +
  geom_col(color = "black", size= .2, width= .8, position= position_dodge(width= .9))+
    geom_text(size = 3, 
            position = position_dodge(width=1), aes(y= amount), vjust =-.8, color= "black")+
  scale_fill_manual(values = pop_colors) 
+
 labs(title =  "<span style = 'font-size:11pt; color:#000000'>Figure 7.</span></p><p><span style = 'font-size:12pt; color:#a21e3c'><b>Composition of American Voters by Educational Detainment and Race</p><p><span style = 'font-size:12pt; color:#a21e3c'><b>and Hispanic Origin: 2016</span></p>",
       subtitle = "(In Percent)",
       y = "",
       x = "",
...
new2 + theme(legend.position="top",legend.text=element_text(margin=margin(r=.2,unit="inch")))+guides(fill=guide_legend(title=NULL))+annotate("text", x= 3.21, y= 62.4, label= "associate's degree", size= 3.1)+coord_cartesian(ylim=c(0,49.5),clip="off")+annotate("text", x= 3.93, y= 62.5, label= "or more", size= 3.1)+coord_cartesian(ylim=c(0,49.5),clip="off")
ggsave("new2.png")
```
